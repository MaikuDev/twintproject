...
def getType(tw):
    ...
    image = tw.select_one("div[data-image-url]")
    video = tw.select_one("div.PlayableMedia-player")
...
def getPreview(tw):
    ...
    if(image is not None):
        preview = image.get_attribute_list('data-image-url')[0]
    elif(video is not None):
        extractor = URLExtract()
        preview = video.get_attribute_list('style')[0]
        preview = extractor.find_urls(preview)[0]
